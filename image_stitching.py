'''
Name: image_stitching.py

Version: 1.0

Summary: Stitch image sets captured by close range camera
...
python3 image_stitching.py --images images/scottsdale --output output.png --crop 1
...
if __name__ == '__main__':
    ...
    mkpath = os.path.dirname(args["images"]) +'/result'
    mkdir(mkpath)
    save_path = mkpath + '/'
    print ("results_folder: " + save_path)
    
    
    # loop over the image paths, load each one, and add them to stich list
    ...
    if status == 0:
        ...
        result_img_path = save_path + args["output"]
    
        cv2.imwrite(result_img_path, stitched)
